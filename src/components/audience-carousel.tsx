 ...
 *
 * It wraps at both ends rather than disabling the arrows: with five items the
 * loop is short enough that a dead button is more annoying than a repeat.
 */
export function AudienceCarousel() {
  ...
  const total = audienceSignals.length;
  ...
  return (
    <div
      ...
      aria-label="Signals that this is for you"
      onKeyDown={(e) => {
    ...
    >
      {/*
        Every signal is laid into the same grid cell, so the box is exactly as
        tall as the longest one and no taller. A guessed min-height either
        stranded the short signals in empty space or clipped the long one, and
        sizing to the active signal alone made the box jump on every click.
      ...
      */}
      <div className="metal-edge relative mx-auto grid w-full max-w-sm rounded-2xl px-6 py-9 sm:px-8 sm:py-11">
        {/*
          Anchored to the box rather than carried inside the slide, so it holds
          the corner while the text moves under it. Out of flow, so it sits in
          the box's top padding without pushing the text down.
        */}
        ...
        {audienceSignals.map((signal) => (
          <Signal key={signal} text={signal} sizer />
        ))}

        {/*
          Keyed on the index so React remounts it on every change, which is what
          restarts the animation. aria-live announces the new text to a screen
          reader, which would otherwise get no signal that anything moved.
        */}
        <Signal
          key={index}
          text={audienceSignals[index]}
          className="slide-in"
          style={
        ...
        <div className="flex items-center gap-2.5">
          {audienceSignals.map((signal, i) => (
            <button
              ...
              type="button"
              onClick={() => jumpTo(i)}
              aria-label={`Go to signal ${i + 1} of ${total}`}
 ...
 * so the same five lines aren't read out under the one that's on screen.
 */
function Signal({
  text,
  sizer = false,
